 ...
 *   id: string | number,
 *   ....}
 *  но не понимаю, что как тут на генериках сделать normalizeData = <T extends Array<?>>
 * 
 * в любом случае, теоретически id может быть number, поэтому охото создать переиспользуемый интерфейс
 */
...
const posts: IPost[] = [
  ...
  const normalizeData = (unnormalizedData: IPost[]): INormalized => {

   
        const init: { [key: id]: IPost} = {}
        const allIds: id[] = []

        const newByID = unnormalizedData.reduce((acc:{ [key: id]: IPost}, item) => {
